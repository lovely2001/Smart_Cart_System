...
int beep = 4; //BUZZER SIGNAL..AT D4
//int Touch = 3;//TOUCHSENSOR SIGNAL D8
int Touch_Butter = 0, Touch_Sugar = 0, Touch_Oil = 0, Touch_Tea = 0;
...
void setup()
{
  ...
  pinMode(beep, OUTPUT);
  pinMode(3, INPUT);
  pinMode(2, OUTPUT); //bUZERR VCC CONNECTION
  pinMode(7, OUTPUT); //TOUCHsENSOR VCC
  pinMode(6, OUTPUT); //TOUCHSENSOR GROUND
...
void loop()
{
  digitalWrite(2, HIGH); //BUZZER ALWAYS HAS VCC HIGH
  digitalWrite(7, HIGH); //TOUCH SENSOR ALWAYS HIGH VCC
  digitalWrite(6, LOW); //TOUCHSENSOR GROUND ALWAYS 0
